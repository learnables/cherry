...
def update(replay,
    ...
    actions = np.clip(actions.detach(), -1.0, 1.0)
    ...
    # QF loss
    qf1_old_pred = qf1(batch.state(), np.clip(batch.action().detach(), -1.0, 1.0))
    qf2_old_pred = qf2(batch.state(), np.clip(batch.action().detach(), -1.0, 1.0))
    ...
    new_actions = np.clip(new_actions.detach(), -1.0, 1.0)
    ...
    if random.random() < 0.05:
        ppt.plot(replay[-1000:].reward().mean().item(), 'cherry true rewards - TSAC2')
